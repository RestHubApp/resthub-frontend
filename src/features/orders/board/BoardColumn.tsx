...
export default function BoardColumn({ title, orders, now, onCharge, onCancel }: BoardColumnProps) {
  return (
    <section aria-label={`${title}: ${String(orders.length)}`} className="flex min-w-0 flex-col gap-3 rounded-2xl bg-muted/60 p-3">
      ...
      {orders.length === 0 ? (
        ...
        <ul className="m-0 flex list-none flex-col gap-3 p-0">
          {orders.map((order) => (
            <li key={order.id}>
              <BoardCard order={order} now={now} onCharge={onCharge} onCancel={onCancel} />
            </li>
          ))}
        </ul>
      )}
